...
input = File.read("input.txt").split.map{|line| line.chars.map(&:to_i)}


# This gets a line of 5X wide
input.map do |line|
  (1..5).reduce(line){|output_line, i| output_line + line.map{|val| (val + i) % 10}}
end

# now how to scale it down?

# Or do I just take the input and scale it that way?
# like..
#
5.times.each do |y|
  5.times.each do |x|
